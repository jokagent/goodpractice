...
class Ion_auth
{
		...
		if ( $this->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			if ($user)
			{
				...
				else
				{
					...
					$this->email->subject('Изменение пароля на '.$this->config->item('site_title','ion_auth'));
	...
	public function register($username, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
	{
		...
		else
		{
            ...
            $q = $this->db->select('temp')->where('id',$id)->get('users')->row();
            $this->db->where('id',$id)->update('users',array('temp'=>'1'));
            $password        = $q->temp;
    ...
    public function get_user_info(){
        $id = $this->get_user_id();
        $this->ion_auth_model->get_user_info($id);
    }
    public function send_email($data){
        $to = $data['identity'];
        $subj = "Изменение пароля";
        $message ="<html><body>Мы получили запрос на изменение Вашего пароля на сайте goodpractice.ru<br>
        Чтобы поменять пароль перейдите по следующей ссылке:<br>
    http://crm.ru/auth/reset_password=".$data['forgotten_password_code']."<br>
    Там Вы сможете ввести новый пароль.<br>
    ЕСЛИ ВЫ НЕ СОБИРАЛИСЬ МЕНЯТЬ ПАРОЛЬ: не волнуйтесь - Ваш пароль НЕ ИЗМЕНИТСЯ пока Вы не перейдете по ссылке. Вы можете просто проигнорировать это сообщение.
    <br>
    Процесс подтверждения необходим, чтобы обеспечить безопасность Вашей личной информации. Спасибо за понимание!</body> </html>";
        $header = "From: Goodpractice.ru <info@crm.ru >\r\n";
