...
//			$this->_render_page('auth/login', $this->data);
	...
	function forgot_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
            echo $this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
            $this->load->view('auth/htmlheader.html');
...
//			$this->_render_page('auth/forgot_password', $this->data);
		...
		else
		{
			...
			$identity = $this->db1->where('email', $this->input->post('email'))->limit('1')->get($config_tables['users'])->row();
	...
	function create_user()
	{
		...
		$this->form_validation->set_rules('surname', $this->lang->line('create_user_validation_fname_label'), 'required|xss_clean');
